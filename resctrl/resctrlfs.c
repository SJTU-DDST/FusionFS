...
static int get_shareable_mask(const char *cache_type, unsigned long *shareable_mask)
{
	...
	snprintf(mask_path, sizeof(mask_path), "%s/%s/shareable_bits", // c00
		 INFO_PATH, cache_type);
...
out:
	...
	// print params in a line
	ksft_print_msg("Writing: bm_pid=%d to ctrlgrp=%s mongrp=%s resctrl_val=%s\n",
		       bm_pid, ctrlgrp, mongrp, resctrl_val);
	if (ret)
...
int write_schemata(char *ctrlgrp, char *schemata, int cpu_no, const char *resource)
{
	...
	// print params in a line
	// ksft_print_msg("write_schemata: ctrlgrp=%s schemata=%s cpu_no=%d resource=%s\n",
	// 	       ctrlgrp, schemata, cpu_no, resource);
	...
	if (strlen(ctrlgrp) != 0) {
		...
		// print controlgroup, RESCTRL_PATH/ctrlgrp in a line
		// ksft_print_msg("write to %s/%s/schemata: %s\n", RESCTRL_PATH, ctrlgrp, controlgroup);
	}
	else {
		sprintf(controlgroup, "%s/schemata", RESCTRL_PATH);
		// print controlgroup, RESCTRL_PATH/schemata in a line
		// ksft_print_msg("write to %s/schemata: %s\n", RESCTRL_PATH, controlgroup);
	}

	schema_len = snprintf(schema, sizeof(schema), "%s:%d=%s;%d=%s\n",
			      resource, 0, schemata, 1, schemata); // IMPORTANT: write to both NUMA nodes
