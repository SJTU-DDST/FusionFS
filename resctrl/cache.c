...
static int print_results_cache_ref(const char *filename, int bm_pid, __u64 llc_value, __u64 ref_value)
{
	FILE *fp;

	if (strcmp(filename, "stdio") == 0 || strcmp(filename, "stderr") == 0) {
		printf("Pid: %d \t LLC_value: %llu \t Ref_value: %llu\n", bm_pid, llc_value, ref_value);
	} else {
		fp = fopen(filename, "a");
		if (!fp) {
			ksft_perror("Cannot open results file");
		...
		fprintf(fp, "Pid: %d \t llc_value: %llu \t ref_value: %llu\n", bm_pid, llc_value, ref_value);
		fclose(fp);
	}
		       ...
		       const char *filename, int bm_pid)
{
	...
	ret = ioctl(pe_fd_ref, PERF_EVENT_IOC_DISABLE, 0);
	if (ret < 0)
		return ret;
	
	ret = read(pe_fd_ref, pe_read_ref, sizeof(*pe_read_ref));
	if (ret == -1) {
		ksft_perror("Could not get perf value");
		return -1;
	}

	return print_results_cache_ref(filename, bm_pid, pe_read->values[0].value, pe_read_ref->values[0].value);
}
...
{
	ksft_print_msg(
		"TEST cat_type: %d, n_bombs: %d , number of bits: %d, miss: %llu, ref: %llu, miss rate: %.1f%%, allocated cache (%s): %zu, span: %d\n",
		uparams->cat_type, uparams->n_bombs, no_of_bits, avg_llc_val, avg_llc_val_ref,
		100 * (float)avg_llc_val / avg_llc_val_ref,
		lines ? "lines" : "bytes", cache_span, FIXED_SPAN);
}
